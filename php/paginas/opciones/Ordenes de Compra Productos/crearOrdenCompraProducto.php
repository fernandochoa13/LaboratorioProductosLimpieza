...
$resultadoNumeroProducto = mysqli_query($conn,"Select * from producto");
$dataProducto = array();
...
if(!isset($_SESSION['total'])) {
    ...
    if(mysqli_num_rows($resultadoNumeroProducto) > 0) {
        while($row = mysqli_fetch_assoc($resultadoNumeroProducto)) {
            $dataProducto[] = $row;
        }
    }
...
<a href="../../home.php"><header class="header_Quimicaroma">
   <img class="LogoQuimicaroma" src="../../../../img/logo.jpg"></img>
    ...
    <div class="form-group">
        <label for="">Seleccione productos:</label> <br>
        <select name="nombreProductos" id="nombreProductos">
            <?php 
            for($i=0;$i<mysqli_num_rows($resultadoNumeroProducto);$i++){
                ?>
                <option value="<?php echo $dataProducto[$i]['idProducto']?>"><?php echo $dataProducto[$i]['nombreProducto']?></option>
                <?php }  ?>
        </select> 
            </div> <br>
            <div class="form-group">
...
<?php  
        if(isset($_POST['cantidad']) && isset($_POST['nombreProductos'])) {
            $idProducto = $_POST['nombreProductos'];
            $cantidad = $_POST['cantidad'];
            $resultadoPrecio = mysqli_query($conn,"SELECT nombreProducto, precio, stock from producto where idProducto=$idProducto");
            $arregloDatos = mysqli_fetch_array($resultadoPrecio);
            $queryProducto = mysqli_query($conn, "INSERT into inventario_has_orden(producto_idproducto, orden_idorden, cantidad) VALUES ('$idProducto', '$id', '$cantidad')");
            $queryVentas = mysqli_query($conn, "SELECT ventas from producto where idProducto='$idProducto'");
            $row = mysqli_fetch_array($queryVentas);
            $nuevasVentas = $row[0] + $cantidad;
            $queryUpdateVentas = mysqli_query($conn, "UPDATE producto set ventas = $nuevasVentas where idProducto=$idProducto");
            if(isset($_SESSION['ids']) && isset($_SESSION['cantidades']) && isset($_SESSION['nombre'])  && isset($_SESSION['precios']) && isset($_SESSION['stocks'])){
                array_push($_SESSION['nombre'], $arregloDatos['nombreProducto']);
                array_push($_SESSION['ids'], $idProducto);
                array_push($_SESSION['cantidades'], $cantidad);
                array_push($_SESSION['precios'], $arregloDatos['precio']);
                array_push($_SESSION['stocks'], $arregloDatos['stock']);
                echo "<div>
                <table id='datatables' class='table'>
                    <thead>
                        <tr>
                            <th style='color: rgb(83, 184, 180)'>ID</th>
                            <th style='color: rgb(83, 184, 180)'>Nombre</th>
                            <th style='color: rgb(83, 184, 180)'>Cantidad</th>
...
<form action="#" method="post">
<div class="botones">
    <input type="submit" name="ordenar" class="btn btn-dark" id="botonRealizarOrden" value="Realizar Orden">

    <input type="submit" name="btn-cancel" class="btn btn-danger" id="botonCancelarOrden" value="Cancelar Orden">
</form>
</div>
