...
class PDF extends FPDF
{
   ...
   function Header()
   {
     ...
     // $consulta_info = $conn->query(" select * from usuario where idusuario=$id ");//traemos datos de la empresa desde BD
      ...
      $this->Cell(100, 10, utf8_decode("REPORTE DE ORDEN DE COMPRA DE PRODUCTOS "), 0, 1, 'C', 0);
      $this->Ln(8);
...
$consulta_reporte_orden = $conn->query("SELECT * from orden INNER JOIN inventario_has_orden ON orden.idorden = inventario_has_orden.orden_idorden where idorden=$id");
$consulta_reporte_insumos = $conn->query("SELECT * from producto");
$arrayInsumos = mysqli_fetch_array($consulta_reporte_insumos);

while ($datos_reporte = $consulta_reporte_orden->fetch_object()) {
   if($datos_reporte->tipoOrden == 1) {
...
$consulta_reporte_insumos = $conn->query("SELECT nombreProducto from producto where idProducto=$datos_reporte->producto_idproducto");
