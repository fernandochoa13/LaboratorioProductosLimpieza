...
if(mysqli_num_rows($resultado) > 0) {
    while($array = mysqli_fetch_array($resultado)) {
       array_push($nombreInsumo, $array['nombreInsumo']);
    ...
    for ($i = 0; $i < $longitud; $i++) {
        for ($j = 0; $j < $longitud - 1; $j++) {
            if ($ventas[$j] < $ventas[$j + 1]) {
                $temporal = $ventas[$j];
                $nombreTemporal = $nombreInsumo[$j];
                $ventas[$j] = $ventas[$j + 1];
                $nombreInsumo[$j] = $nombreInsumo[$j + 1];
                $ventas[$j + 1] = $temporal;
                $nombreInsumo[$j + 1] = $nombreTemporal;
        }
    } } 

    for($i = 0;$i < 5;$i++){
            $Masvendidos[$i] = $ventas[$i];
            $InsumosMasVendidos[$i] = $nombreInsumo[$i];
    }
    ?>
<!DOCTYPE html>
<html lang="en">
...
<nav>
    ...
    <h1 class="titulo-Seccion">Los 5 insumos más pedidos</h1> <br><br>
    </div>
        <div class="container">
    ...
    <script>
        ...
        let lineChart = new Chart(CHART, {
            type: "line",
            data: {
                labels: [<?php echo '"'.implode('","',  $InsumosMasVendidos ).'"' ?>],
                datasets: [{
                    label: 'Ventas',
