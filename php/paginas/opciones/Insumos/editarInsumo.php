...
$queryResultado = mysqli_query($conn,"SELECT * FROM insumo where idInsumo='$idInsumo'");
...
<nav>
   ...
   <input type="text" class="form-control" placeholder="<?php echo $rows['nombreInsumo'] ?>" name="nombreField" required> <br> <br>
</div>
<div class="form-group">
   <label for="">Precio:</label>
   <input type="number" class="form-control" placeholder="<?php echo $rows['precio']?>" name="precioField" required> <br> <br>
</div>
   <input type="submit" class="btn" name="botonUpdate">
   </form>
</div>
</section>
<footer>
...
<?php
if(isset($_POST['botonUpdate']) && !empty($_POST['nombreField']) && !empty($_POST['precioField'])) {
    $nombreInsumo = $_POST['nombreField'];
    $precioInsumo = $_POST['precioField'];
    $resultadoQuery = mysqli_query($conn,"UPDATE insumo SET nombreInsumo = '$nombreInsumo', precio = '$precioInsumo' WHERE idInsumo = $idInsumo");
    if(!$resultadoQuery) {
